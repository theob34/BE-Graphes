...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    public void remove(E x) throws ElementNotFoundException {
        ...
        int indexToRemove=0;
        indexToRemove = this.array.indexOf(x) ;
        //while(indexToRemove<this.currentSize && x.compareTo(this.array.get(indexToRemove))!=0){ 
        //   indexToRemove++;
        //}
        if (indexToRemove>=this.currentSize){
            throw new ElementNotFoundException(x);
        }

        /* Echanger le dernier élement avec celui que l'on veut enlever */
        E lastItem = this.array.get(this.currentSize-1);
        this.arraySet(indexToRemove, lastItem);

        /* Réevaluer le tas */
        percolateUp(indexToRemove); // Percolate up l'element ajouté
        percolateDown(indexToRemove); // Percolate down l'element ajouté 
